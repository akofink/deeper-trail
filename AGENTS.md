...
- Do not stop after a successful merge alone. Before you consider the task done, verify `git status --short --branch` is clean in `main`, `git branch --no-merged main` shows no leftover project branches other than your active task branch if it is still in progress, and `git worktree list` no longer includes merged task worktrees.
- If you discover staged or unstaged changes sitting in `main` that exactly match an unmerged task branch, treat that as an incomplete integration. Finish the merge or explicitly report why Git metadata writes are blocked; do not leave duplicate branch work staged in the primary checkout.
- If the current environment blocks writes under `.git` or `.git/worktrees` and therefore prevents `commit`, `merge`, `stash`, `branch -d`, or `worktree remove`, say so explicitly in your final handoff with the exact failing command and path. A blocked sandbox is not a reason to leave silent repo hygiene debt behind.
