...
} from '../src/game/runtime/browserShellBootstrapSession';
...
describe('browserShellBootstrapSession', () => {
  it('loads modules, wires scene renderers, and creates the runtime controller', async () => {
    const shellWindow = {
    ...
    const result = await createBrowserShellBootstrapSession(shellWindow, documentHost, {
      createApp,
    ...
    expect(result.app).toBe(appResult.app);
    expect(result.runtime.drawInitialScene).toBe(drawInitialScene);
    ...
    result.rendererBindings.renderMapScene({ scene: 'map' } as RuntimeState);
    result.rendererBindings.renderRunScene({ scene: 'run' } as RuntimeState);
