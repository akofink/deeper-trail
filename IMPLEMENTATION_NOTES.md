...
- Completed expeditions can now seed one pending legacy echo into the next fresh world through `goalSignal.ts` + `shellControl.ts`, with the one-shot carry-over surfaced in map/debug helpers and consumed on the next expedition's first successful travel.
