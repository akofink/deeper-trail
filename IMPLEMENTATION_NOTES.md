...
- The browser smoke runner can now either emit one-shot timing artifacts (`--timings-out`) or append durable timing history samples (`--timing-history-out`), so repeated local/CI replay runs can keep a simple JSON trail without changing the deterministic replay flow itself.
