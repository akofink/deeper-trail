...
- The committed Playwright smoke path should stay deterministic: prefer fixed seeds plus `window.render_game_to_text` / `window.advanceTime(ms)` over adding separate test-only gameplay controls, keep the seed/biome matrix centralized in `scripts/e2e/fullObjectiveLoop.js`, and treat the debug snapshot's exported world graph as the route-planning source for browser automation.
- The biome smoke suite may reuse one launched browser across all selected seeds to reduce startup cost, but each smoke should still run in its own fresh browser context/page so state never leaks between deterministic replays.
- The smoke runner should emit per-smoke timing plus one compact suite timing summary so slow biome regressions are visible without switching to verbose profiling.
- When you need to keep timing history outside terminal logs, run `node scripts/e2e/fullObjectiveLoop.js --smoke all --timings-out <path>.json`. The runner will create parent directories and write the same per-smoke + total timing summary as JSON.
- When you want durable timing samples across multiple smoke runs, add `--timing-history-out <path>.json`. The runner will append `{ recordedAt, selection, totalDurationMs, timings[] }` entries to a JSON history file while still supporting the one-shot `--timings-out` artifact.
